...
const WORLD_SIZE := Vector3(25, 25, 25)
...
func _ready() -> void:
	...
	if SIMULATE_GPU:
		...
		compute3d.setup("res://compute_shaders/boid_simulation_3d_alt.glsl", boid_pos, boid_vel, boid_data.get_data(), boid_quat.get_data())
		compute3d.update([
			NUM_BOIDS, IMAGE_SIZE,
			friend_radius, avoid_radius,
			min_vel, max_vel,
			steer_factor, alignment_factor,
			cohesion_factor, separation_factor,
			WORLD_SIZE.x, WORLD_SIZE.y, 0.0
		])
...
func _process(delta: float) -> void:
	if SIMULATE_GPU:
		...
		compute3d.update([
			...
			steer_factor, alignment_factor,
			cohesion_factor, separation_factor,
			WORLD_SIZE.x, WORLD_SIZE.y, delta
		])
